...
if (isset($_POST['ajouter'])) {
    $stmt = $pdo->prepare('INSERT INTO commentaire (id_com, contune, id_ressource) VALUES (?, ?, ?)');
    $stmt->execute([null, $_POST['contenu'] ?? '', $id]);
...
<div class="container my-5">
    ...
    <tbody>
        <?php foreach($commentaires as $com): ?>
            <tr data-comment-id="<?= $com['id_com'] ?>">
                <td><?= nl2br(htmlspecialchars($com['contune'])) ?></td>
                <td>
                    <?php if ( (isset($_SESSION['role']) && $_SESSION['role'] == 'admin')): ?>
                        <a href="modifier_commentaire.php?id_com=<?= (int) $com['id_com'] ?>&id_ressource=<?= $id ?>" class="btn btn-sm btn-warning">Modifier</a>
                        <a href="supprimer_commentaire.php?id_com=<?= (int) $com['id_com'] ?>&id_ressource=<?= $id ?>" class="btn btn-sm btn-danger" onclick="return confirm('Voulez-vous vraiment supprimer ce commentaire ?')">Supprimer</a>
                    <?php else: ?>
